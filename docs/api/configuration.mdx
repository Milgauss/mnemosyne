...
There are **116 configuration keys**. A further
**50 environment variables** are read directly from the
...
| `media_allow_private_urls` | `MNEMOSYNE_MEDIA_ALLOW_PRIVATE_URLS` | `false` | no | Let the mnemosyne_remember_media tool fetch URLs that resolve to loopback, private or link-local addresses. |
| `media_allowed_paths` | `MNEMOSYNE_MEDIA_ALLOWED_PATHS` | *(unset)* | no | Directories the mnemosyne_remember_media tool may read local files from (os.pathsep or comma separated). Empty: tool calls cannot name local files. |
